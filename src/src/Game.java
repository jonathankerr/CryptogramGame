...
{ 
	...
	private void getTop10() 
	{
            ...
            for (int i = 0; i < files.length; i++) { 
            	if(files[i].getName().equals("prevGame")) {
            		//do nothing with this file
            			...
            			if(x3 > 0) { //if user has actually completed a cryptogram we add to list
        ...
        loop:
	        for(Player str: allPlayers) {
	        	if(count == 9) {
		        	...
		        	count++;
	        		if(str.getCompletedCryptograms() > 0) {
        ...
        if(listSize == 0) {
        	System.out.println("No players have managed to complete a cryptogram yet.");
        }
